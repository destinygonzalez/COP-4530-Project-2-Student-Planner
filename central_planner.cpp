...
void plannerLogic:: addEntry(string name, string deadline){ //call function to make new entry and add it to the list
    ...
    Entry addEntry(name, deadline); //made new entry with name and dealine
...
void plannerLogic:: deleteEntry(){ //call the function that will delete the most recent entry in the list
    ...
    if (entryStack.empty()) cout << "Nothing to delete!" << endl; return; //if stack is empty, cannot delete 
...
if (undoButton.isEmpty()) cout << "Nothing we can undo!" << endl; return; //if even the undo stack is empty, we cannot undo anything

entryStack = undoButton.pop(); cout << "Latest delete was undone" << endl; //delete the recent most entry from the undo stack and make it the current stack
...
void plannerLogic:: showEntries(){ //call the function that will show all the entries to the user
    ...
    if (entryStack.empty()) cout << "No tasks yet, add some and start planning!" << endl; return; //if empty, no tasks to show
